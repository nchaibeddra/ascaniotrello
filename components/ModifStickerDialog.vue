<template>
    <div v-if="$store.state.modifStickerDialog" class="fixed w-full min-h-screen opacity-90 bg-gray-200">
        <div class="mt-36 border bg-secondary rounded-xl w-1/3 mx-auto p-5">
            ...
            <select v-model="selectedCity" class="w-full  mt-2 rounded px-2">
                <option selected v-for="(city, index) in foundCity" :key="index" :value="city.nom">{{ city.nom }}
                </option>
            </select>
            <input v-model="inputDate" class="w-full px-2  mt-2 rounded" type="date">
            <button @click="modifyStep(inputStep, selectedCity, inputDate)"
...
export default {
    ...
    methods: {
        ...
        modifyStep(inputStep, selectedCity, inputDate) {
            this.$parent.modifyStep(inputStep, selectedCity, inputDate, this.$store.state.stickerData)
        },
